...
class BMI:
    ...
    def populate(_inputObj, _table_one):
        bmi = round(BMI.bmi_calculator(_inputObj['WeightKg'], (_inputObj['HeightCm'] * 0.01)), 2)
        #print("Bmi : ", bmi)
    ...
    def calculate(self):
        _input_records = BMI.fetch_data_from_file("input.txt")
        _table_one = BMI.fetch_data_from_file("table_one.txt")
        print('--start--')
        output_array=[]
        for _inputObj in _input_records:
            #print(_inputObj)
            BMI.populate(_inputObj, _table_one)
            #print(_inputObj)
            BMI.append_output_file(_inputObj,output_array)

        #print(output_array)
        print('..saving bmi calculate data into output.txt ')
        count = BMI.countOverweight(output_array)
